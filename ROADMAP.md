...
The toolkit ships **24 interconnected skills** and **12 domain references** as a `SKILL.md`-based package that works natively in Claude Code, Codex CLI, Gemini CLI, Cursor, and Windsurf. The CLI is a single Node.js file with **zero runtime dependencies** — `npm install -g @kudusov.takhir/ba-toolkit` and you have the whole pipeline available offline.
...
- **CHANGELOG hygiene CI check.** A small script that validates section ordering, version monotonicity, and compare-link presence on every pull request. Catches the manual numbering drift that has been hand-fixed three times in the recent release history. Closes a real recurring pain point.
...
| 3.8.0 | 2026-04-10 | **Group C senior-BA audit pass on `/discovery`, `/principles`, `/research`, `/handoff`, `/implement-plan`, `/export`** — bookend skills. ~26 Critical and High findings applied. Highlights: full Michael Nygard ADR format with Drivers and Alternatives Considered for `/research`, expanded `/handoff` artifact inventory covering all 15 pipeline stages plus 6 cross-cutting tools, formal Sign-off section for `/handoff`, principles Definition of Ready section synchronised with the v3.5.0+ template fields across every artifact type, new ISO/IEC 25010 alignment in the principles NFR baseline, new Testing Strategy section in `/principles` (the principled answer to the TDD-skill question), Hypotheses → Brief Goals retrospective traceability in `/discovery`, Observability + CI/CD + Secret-management slots in `/implement-plan`, full v3.5.0+ stories template field support in `/export`. After this pass, 22 of 24 shipped skills carry the senior-BA improvements. |
| 3.7.0 | 2026-04-10 | **Group B senior-BA audit pass on `/trace`, `/analyze`, `/clarify`, `/estimate`, `/glossary`, `/risk`, `/sprint`** — cross-cutting utilities. ~33 Critical and High findings applied. Highlights: full ISO 31000 + PMBOK 7 alignment for `/risk` with the Velocity axis and treatment-strategy classification, Cone of Uncertainty plus confidence bands for `/estimate`, IEEE 830 §4.3 quality attributes mapped to `/analyze` finding categories, definition-quality discipline (Aristotelian form, ISO 1087-1) for `/glossary`, structured table output and ripple-effect check for `/clarify`, bidirectional traceability and `/implement-plan` integration for `/trace`, focus-factor and ceremonies-aware net velocity for `/sprint`. Two utility templates rewritten from concrete worked examples back to placeholder convention. |
| 3.6.0 | 2026-04-10 | **Group A senior-BA audit pass on `/usecases`, `/ac`, `/nfr`, `/datadict`, `/apicontract`, `/wireframes`, `/scenarios`** — pipeline core. ~32 Critical and High findings applied. Standards conformance (Cockburn use cases, ISO 25010 NFRs, OpenAPI 3.x shape for the API contract), inline-vs-standalone drift fixes, "why" fields (Source / Owner / Sensitivity / Verification) on every artifact element, cross-artifact forward-traceability matrices in every template, canonical 8-state list for `/wireframes`, logical-vs-physical scope split for `/datadict`, document-control metadata on all 7 templates. |
| 3.5.0 | 2026-04-09 | **Pilot skill audit on `/brief`, `/srs`, `/stories`** — 14 Critical and High findings applied. `/brief`: numbering bug fixed, mandatory "Out of Scope" section, Constraints split from Assumptions (BABOK v3 discipline), 4 new required topics (buyer-vs-user separation, decision-making authority, regulatory pre-screening, failure criteria). `/srs`: FR template gains Source / Verification / Rationale per IEEE 830 §7, new Brief Goal → FR traceability matrix, §3 grouped by feature area with reserved ID ranges, 5 new universal required topics. `/stories`: SKILL.md inline template removed in favour of standalone (drift fix), INVEST is now an explicit quality gate, named Persona / Business Value Score / Depends on fields added, FR → Story coverage matrix. |
| 3.4.1 | 2026-04-09 | Content-hygiene audit pass after the v3.4.0 ship — 5 drift / staleness fixes flagged by a cross-file consistency check (`interview-protocol.md` enumerated list missing `/discovery`, `/discovery` SKILL.md still claiming "9 supported domains" with the pre-v3.3.0 enumeration, `/wireframes` and `/scenarios` hardcoding "Pipeline complete" against the v3.1.0 single-source-of-truth rule). |
| 3.4.0 | 2026-04-09 | **New `/implement-plan` skill — sequenced implementation plan for AI coding agents.** New stage 12 of the BA Toolkit pipeline, runs after `/handoff` and produces `12_implplan_{slug}.md` — a phase-by-phase plus DAG-by-task plan an AI coding agent (Claude Code, Cursor, Codex) can execute step by step. Phase ladder: Foundation → Data → Auth → Core → API → UI → Integrations → Quality → Validation. Tech stack resolved from `/research` if present, otherwise via a calibration interview. Methodology-neutral by design — tasks reference AC and carry a Definition of Done, but do not embed test specs or other process opinions. |
| 3.3.0 | 2026-04-09 | **Three new domain references — EdTech, GovTech, AI / ML.** The shipped catalog grows from 9 to 12 first-class industries plus the `custom` fallback. Each reference follows the established 9-section structure (one section per pipeline interview-phase skill plus a domain glossary) and matches the depth of the existing references. EdTech covers K-12, higher ed, MOOC, corporate L&D, language learning. GovTech covers citizen e-services, permits, tax, benefits, public records. AI / ML covers LLM apps, RAG, agents, model serving, fine-tuning, evals. |
