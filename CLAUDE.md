...
- **Major** — renaming/removing a skill, breaking change to `output/{slug}/` file naming, incompatible change to a CLI flag or subcommand.
...
- **Cursor/Windsurf format conversion:** `SKILL.md` → `.mdc` happens in `skillToMdc()` in `bin/ba-toolkit.js`. `.mdc` requires a different YAML frontmatter (`description`, `alwaysApply`). A plain rename is not enough.
